(function() {
	...
	async function fetchTabs() {
		return (await chrome.tabs.query({})).filter((t) => t.url && !t.url.startsWith("chrome://")).map((t) => ({
			id: `tab-${t.id}`,
			title: t.title || t.url || "",
			url: t.url,
			source: "tab",
	...
	async function fetchAll() {
		...
		for (const item of history) seen.set(item.url, item);
	...
	*/
	...
	var MAX_BITS = 32;
	function search$1(text, pattern, patternAlphabet, { location = Config.location, distance = Config.distance, threshold = Config.threshold, findAllMatches = Config.findAllMatches, minMatchCharLength = Config.minMatchCharLength, includeMatches = Config.includeMatches, ignoreLocation = Config.ignoreLocation } = {}) {
	...
	*/
	...
	chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
		...
		if (message.type === "OPEN_URL") {
			if (message.newTab) chrome.tabs.create({ url: message.url });
			else chrome.tabs.update({ url: message.url });
			return;
		}
	});
